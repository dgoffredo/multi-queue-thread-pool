...
void JobQueue::run_front_job(ThreadPool& pool) {
  ...
  pool.enqueue([this, &pool]() { run_front_job(pool); });
}

JobQueue::JobQueue() : job_pending(false), sealed(false), paused(false) {}
...
void JobQueue::wait_for_current() {
  ...
  current_job_done.wait(lock, [this]() { return !job_pending; });
}

void JobQueue::wait_for_empty() {
  std::unique_lock<std::mutex> lock{mutex};
  empty.wait(lock, [this]() { return jobs.empty(); });
}
...
QueueHandle::QueueHandle(std::list<JobQueue>::iterator queue) : queue(queue) {}

// class MultiQueueThreadPool
// --------------------------

MultiQueueThreadPool::MultiQueueThreadPool(ThreadPool& pool) : pool(pool) {}
...
int MultiQueueThreadPool::enqueue(QueueHandle handle,
                                  std::function<void()> job) {
  return handle.queue->enqueue(std::move(job), pool);
}

void MultiQueueThreadPool::seal(QueueHandle handle) { handle.queue->seal(); }

void MultiQueueThreadPool::unseal(QueueHandle handle) {
  handle.queue->unseal();
}

void MultiQueueThreadPool::pause(QueueHandle handle) { handle.queue->pause(); }

void MultiQueueThreadPool::unpause(QueueHandle handle) {
  handle.queue->unpause(pool);
}

void MultiQueueThreadPool::stop(QueueHandle handle) { handle.queue->stop(); }

void MultiQueueThreadPool::flush(QueueHandle handle) { handle.queue->flush(); }
